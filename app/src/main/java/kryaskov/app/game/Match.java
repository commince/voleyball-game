...
public class Match { //Отвечает за последовательность и наполнение событий матча
    ...
    private void createTeams() {
        team1 = createATeam();
        if(teamBuilder instanceof ManualTeamBuilder && UserInput.userInput.askForSave()) new DBTeamSaver().saveTeam(team1);
        System.out.println("");
        ...
        if(teamBuilder instanceof ManualTeamBuilder && UserInput.userInput.askForSave()) new DBTeamSaver().saveTeam(team2);
        System.out.println("");
    }

    private Team createATeam() {
        teamBuilder = UserInput.userInput.creationMethod(" команды").equals("y") ? new DBTeamBuilder() : new ManualTeamBuilder();
